...
(function ($) {
    ...
    $.paramquery.tableToArray = function (tbl) {
        ...
        var rObj = { data: data, colModel: colModel };
        return rObj;
    };
...
(function ($) {
    ...
    fnPG._refresh = function () {
        ...
        if (this.options.totalRecords >= 0) {
            ...
            var begIndx =this.options.totalRecords==0?0: (currentPage - 1) * rPP;
            var endIndx = currentPage * rPP;
            if (endIndx > totalRecords) {
                endIndx = totalRecords;
            }
            var strDisplay = this.options.strDisplay;
            strDisplay = strDisplay.replace("[0]", this.options.totalRecords==0?0:(begIndx + 1));
...
//            toolTip = that.data;
        ...
        if (wrap == false)
            cls += " pq-wrap-text";
        var str = "<div class='" + cls + "' title='"+toolTip+"'>" + dataCell + "</div>";
        if ($td != undefined) {
            $td.html(str);
        }
        return str;
    ...
    _pG._generateRow = function (rowData, rowIndx, thisColModel, noColumns, hidearrHS1, offset, const_cls, buffer, objP) {
        ...
        for (var col = 0; col < noColumns; col++) {
            ...
            var str = "<td class='" + cls + "' style='" + strStyle + "' " + indxStr + " >" + this._renderCell(objRender) + "</td>";
            buffer.push(str)
        }
        for (var k = 0; k < hidearrHS1.length; k++) {
            var col = hidearrHS1[k];
    ...
    fn._findCellFromEvt = function (evt) {
        ...
        if ($td == null || $td.length == 0) {
            var rObj = { rowIndxPage: null, colIndx: null, $td: null };
            return rObj;
        }
        else {
    ...
    fn.remoteRequest = function (callback_fn) {
        ...
        this.xhr = $.ajax({
            url: url,
            dataType: DM.dataType || "json",
            async: true,
            cache: DM.cache,
            type: DM.method || "POST",
            timeout: DM.timeout || 30 * 1000,
            data: dataURL,
            ...
            error: function (jqXHR, textStatus, errorThrown) {
                ...
                if(jqXHR.statusText == "timeout"){ errorStr = "网关超时！";}
                if (typeof DM.error == "function") {
                    var errObj = { XMLHttpRequest: jqXHR, textStatus: textStatus, errorThrown: errorThrown, errorStr: errorStr, responseText: jqXHR.responseText };
                    DM.error(errObj);
                }
            }
        });
    ...
    fn.getViewPortRowsIndx = function () {
        var rObj = { beginIndx: this.init, endIndx: this['final'] };
        return rObj;
    }
    ...
    fn.getEditCell = function () {
        if (this.$td_edit) {
            var rObj = { $td: this.$td_edit, $cell: this.$div_focus };
            return rObj;
        }
        else {
            return null;
    ...
    fn._editCell = function ($td) {
        ...
        if (column.editor) {
            ...
            $cell.html("<div  tabindx='0' class='pq-grid-editor-default' style='white-space:nowrap;text-overflow:ellipsis;overflow:hidden;'></div>");
            var that = this;
    ...
    fn._incrIndx = function (rowIndxPage, colIndx) {
        ...
        if (colIndx == null) {
            ...
            var rObj = { rowIndxPage: rowIndxPage };
            return rObj;
        }
        var column;
        do {
        ...
        var rObj = { rowIndxPage: rowIndxPage, colIndx: colIndx };
        return rObj;
    }
    ...
    fn.nestedCols = function (colMarr, _depth, _hidden) {
        ...
        var rObj = { depth: new_depth, colModel: arr, colSpan: colSpan, width: width, childCount: childCount };
        return rObj;
    };
    ...
    fn._refreshOtherTables = function () {
        ...
        for (var i = 0; i < this.tables.length; i++) {
            ...
            for (var col = 0; col < noColumns; col++) {
                ...
                var str = "<td class='" + cls + "' style='" + strStyle + "' pq-col-indx='" + col + "'>" + this.cTable._renderCell(objRender) + "</td>";
                buffer.push(str);
            }
            for (var k = 0; k < hidearrHS1.length; k++) {
                var col = hidearrHS1[k];
