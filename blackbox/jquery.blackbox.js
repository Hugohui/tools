...
(function () {
     ...
     * @param _delay_appear
     */
    BlackBox.fn.prompt = function (text, callback, options, _delay_appear, placeholder) {
        if (arguments.length === 0) return;
        var args = _getArgs.apply(this, arguments);
        ...
        var $BlackBox = $("#BlackBox");
        $BlackBox.append('<div class = "system Inner" id="prompt' + _getNowID.call(this) + '"><p>' + args[0] + '</p><input id="boxInput" style="height:30px;width:460px" placeholder="必填，少于20字"></div>');
    ...
    var _setButton = function (onSubmit, onCancel, options) {
        ...
        if (onCancel) {
            var cancel_text = options.cancel || '取消';
            $BlackBoxAction.append('<button class="cancel">' + cancel_text + '</button>');
